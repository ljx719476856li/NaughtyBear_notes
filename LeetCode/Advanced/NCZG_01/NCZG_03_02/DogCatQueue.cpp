...
Pet DogCatQueue::PollAll()
{
	...
	if (!m_cat.empty() && !m_dog.empty())
	{
		...
		{
			res = Cat(*static_cast<Cat*>(m_cat.front().GetPet()));
			//res = m_cat.front().GetPet();
			m_cat.pop();
		}
		else
		{
			res = Dog(*static_cast<Dog*>(m_dog.front().GetPet()));
			m_dog.pop();
		}
	}
	else if (!m_cat.empty())
...
Cat DogCatQueue::PollCat()
{
	...
	Cat res(*static_cast<Cat*>(m_cat.front().GetPet()));
...
Dog DogCatQueue::PollDog()
{
	...
	Dog res(*static_cast<Dog*>(m_dog.front().GetPet()));
	//Dog* dog = static_cast<Dog*>(m_dog.front().GetPet());
	m_dog.pop();
	return res;
}
